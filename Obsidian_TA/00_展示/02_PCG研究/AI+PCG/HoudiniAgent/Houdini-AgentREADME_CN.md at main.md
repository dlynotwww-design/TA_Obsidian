---
title: "Houdini-Agent/README_CN.md at main"
source: "https://github.com/Kazama-Suichiku/Houdini-Agent/blob/main/README_CN.md"
author:
published:
created: 2026-06-21
description: "Houdini Agent - DCC Asset Manager with AI capabilities - Houdini-Agent/README_CN.md at main · Kazama-Suichiku/Houdini-Agent"
tags:
  - "clippings"
---
## Houdini Agent
...
| 提供商 | 模型 | 说明 |
| --- | --- | --- |
| **DeepSeek** | `deepseek-v4-flash` 、 `deepseek-v4-pro` 、 `deepseek-chat` *、 `deepseek-reasoner`* | V4：显式 thinking 参数 + reasoning\_effort；\*旧模型 2026/07/24 废弃 |
...
| `update_todo` | 更新任务状态（pending / in\_progress / done / error） |
...
- **v2.0.1** — **就地更新后正确进入新 QML 界面** ：修复"更新到 2.0 却还是以前的 UI"。旧版(1.5.x)的 Houdini 内嵌自动更新通过 `houdini_agent.main.show_tool()` 重启，而 2.0.0 里它是旧 PySide 入口——就地更新的用户因此停在旧界面。现在 `main.show_tool()` 默认转发到内嵌 QML 界面（ `HAGENT_UI=legacy` 可回退）；官网下载的仍是独立安装包（启动方式不同）。另修复打包后更新器误报"当前版本 0.0.0"（健壮的 VERSION 查找），并新增离线 pytest 测试套件 + CI。
...
- **v1.5.1** — **Wrangle run\_over 修复** ：修复 wrangle `class` （run\_over）参数映射，匹配 Houdini 参数菜单值。
- **v1.5.0** — **自定义提供商 + capture\_viewport** ：新增 **Custom Model** 提供商 — 用户可配置 URL、API Key、模型名、上下文限制、Vision 和 Function Calling 支持；兼容任何 OpenAI 兼容端点（LM Studio、vLLM 等）。新增 `capture_viewport` 工具用于视觉验证 — 截取 3D 视口截图，返回 base64 JPEG 给视觉模型或保存到文件。 `execute_python` 新增 stop-event 保护和 30s 超时。Agent 模式移除基于意图的工具过滤。
...
- **v1.1.2** — **节点布局工具** ：新增 `layout_nodes` 工具，支持 3 种策略 — `auto` （智能，使用 NetworkEditor.layoutNodes 或 moveToGoodPosition）、 `grid` （固定宽度网格排列）、 `columns` （基于拓扑深度的分列布局，可调间距）。新增 `get_node_positions` 查询节点坐标。 **布局工作流规则** ：System Prompt 强制执行顺序：创建节点 → 连接 → verify\_and\_summarize → layout\_nodes → create\_network\_box（布局必须在 NetworkBox 之前，因为 fitAroundContents 依赖节点位置）。 **Widget 闪烁修复** ： `CollapsibleSection` 和 `ParamDiffWidget` 将 `setVisible` 调用移到 `addWidget` 之后，防止无 parent 窗口闪烁。
...
- **v1.0.0** — **图片/多模态输入** ：支持粘贴/拖拽/文件选择器附加图片，发送前缩略图预览，模型视觉能力自动检测。 **Wrangle run\_over 指导** ：系统提示词新增 VEX 执行上下文选择规则（防止错误的 run\_over 模式）。 **新增模型** ： `gpt-5.3-codex` 、 `claude-opus-4-6-normal` 、 `claude-opus-4-6-kiro` 。 **代理 tool\_call 修复** ：健壮拆分中转服务拼接的 `{...}{...}` arguments。 **旧模块清理** ：启动时自动清理 `HOUDINI_HIP_MANAGER` 残留模块。
