---
title: "Unreal Engine 5 — 3D Gaussian Splatting"
source: "https://zhuanlan.zhihu.com/p/1908283220954420605"
author:
  - "[[Italink]]"
published:
created: 2026-06-09
description: "Github 博客： https://italink.github.io/ModernGraphicsEngineGuide/04-UnrealEngine/16.3DGaussianSplatting/ 三维高斯泼溅（3D Gaussian Splatting，3DGS） 是一种用于实时辐射场渲染的计算机图形学技术，它是…"
tags:
  - "clippings"
---
风中翠竹 等 191 人赞同了该文章
...
- 1993 年的 [SIGGRAPH](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=SIGGRAPH&zhida_source=entity) 论文 [《View interpolation for image synthesis》](https://link.zhihu.com/?target=https%3A//dl.acm.org/doi/10.1145/166117.166153) 中首次提出图像合成的多视图插值概念，定义了视图合成问题，引领了基于图像的渲染（IBR）领域。当时计算机图形渲染受几何、照明和材质输入模型质量限制，IBR 提供了通过组合照片般逼真的输入图像创建新图像的替代方法。随后，视图合成和基于图像的渲染相关工作迅速增长，出现了光场渲染、光栅图、分层深度图等经典论文
- 2020 年的 [ECCV](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=ECCV&zhida_source=entity) 的论文 [《NeRF: Representing Scenes as Neural Radiance Fields for View Synthesis》](https://link.zhihu.com/?target=https%3A//www.matthewtancik.com/nerf) 提出了一种基于神经网络的方式，它能够从少量的二维图像中生成高质量的新视角图像，开创了三维重建的新局面。Nerf使用全连接的神经网络（多层感知器 MLP）表示 3D 场景，通过沿着相机射线查询 5D 坐标并利用体积渲染技术将输出颜色和密度投影到图像上以合成视图，还引入了位置编码使 MLP 能恢复高频函数从而产生高质量结果
- 2023年的 SIGGRAPH 论文 [《3D Gaussian Splatting for Real-Time Radiance Field Rendering](https://link.zhihu.com/?target=https%3A//repo-sam.inria.fr/fungraph/3d-gaussian-splatting/) 》提出了一种使用三维高斯分布作为图形基元的场景表示方式，它是 NeRF 之后的重要突破，兼具渲染效率与重建质量，迅速成为了学术研究的热点。

在影视和游戏行业，通常会采用 **网格（Mesh）** 做为图形主流的渲染载体，它的显著特征是： **图元 （Primitive）** （通常是三角形）+ **纹理（Texture）**

![](https://pica.zhimg.com/v2-72cc3be3f4914d6e2a861641065804d6_1440w.jpg)

这是因为网格的结构具有较高的灵活性，便于迭代编辑，这也使得现代图形渲染管线的发展与之密不可分，并衍生出了大量成熟的工具链，形成了标准化的行业工作流。

![](https://pic1.zhimg.com/v2-d3e0da599992133b0aa8e9204655b656_1440w.jpg)
...
![动图封面](https://pic3.zhimg.com/v2-bd156db2a889677a3bdbba7ed66ce03c_b.jpg)

上图所示的点云正是借助开源工具 **[Colmap](https://link.zhihu.com/?target=https%3A//github.com/colmap/colmap)** 使用 **SFM（ [Structure from Motion](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=Structure+from+Motion&zhida_source=entity) ，运动恢复结构）** + **MVS（ [Multi-View Stereo](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=Multi-View+Stereo&zhida_source=entity) ，多视图立体视觉）** 生成的，我们只需输入多个视角的图像：

![](https://pic2.zhimg.com/v2-fb4e7fe9e00e7af7fc797e15a7a70873_1440w.jpg)

SFM过程会依次执行以下几个步骤：

- **特征提取（Feature Extraction）** ：加载输入图像并进行预处理，提取出各个图像的所包含的特征点。
![](https://pic2.zhimg.com/v2-f8bb8b556d63a999b9de3f951c2e5b07_1440w.jpg)

- **特征匹配（Feature Matching）** ：在不同图像间寻找相同的特征点，建立对应关系。
![](https://picx.zhimg.com/v2-51d9665edb80085a2f0acc0a2c65b1f9_1440w.jpg)

- **增量式重建（Mapper）** ：基于匹配关系，逐步恢复相机位姿和稀疏点云。
![](https://pic3.zhimg.com/v2-b1ff8f9eb1dfe2a92c41dcb028f74af2_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-abf517a3c27cfe4e56264c8286c18f1b_1440w.jpg)

- **深度图融合（Fusion）** ：将多幅图像的深度图合并为一个全局一致的密集点云，消除冲突并填充空洞。
![](https://picx.zhimg.com/v2-5a7e13c3335ef367bb7ab2654d20c9d7_1440w.jpg)

但相同的图形结构，点云通常具有庞大的数据量，上述场景的原始模型具有 `4,876` 个三角形，转换为较低精度的点云之后却仍然包含 `429,113` 个点，因此三维重建领域仍然需要考虑将点云转换为Mesh，这是一些常见的做法：

- **泊松重建（ [Poisson Reconstruction](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=Poisson+Reconstruction&zhida_source=entity) ）** ：将点云视为带符号的距离函数（隐式曲面）的采样点，通过求解泊松方程提取等值面（即 Mesh 表面）。
- **移动最小二乘法（MLS，Moving Least Squares）** ：通过局部加权最小二乘法拟合每个点的切平面，生成隐式曲面，再通过 [Marching Cubes](https://zhida.zhihu.com/search?content_id=257985276&content_type=Article&match_order=1&q=Marching+Cubes&zhida_source=entity) 算法提取 Mesh
...
- **深度学习方法** ：利用神经网络直接从点云预测 Mesh 的顶点和拓扑，适合自动化处理或复杂形状重建。如 [AtlasNet](https://link.zhihu.com/?target=https%3A//github.com/ThibaultGROUEIX/AtlasNet) ， [PointToMesh](https://link.zhihu.com/?target=https%3A//github.com/nywang16/Pixel2Mesh) ， [Mesh R-CNN](https://link.zhihu.com/?target=https%3A//github.com/facebookresearch/meshrcnn) ， [ConvONet](https://link.zhihu.com/?target=https%3A//github.com/autonomousvision/convolutional_occupancy_networks) 等。
...
![](https://pic4.zhimg.com/v2-9359ec8c2405d1dd6508dd845a49c1ad_1440w.jpg)
...
![](https://pica.zhimg.com/v2-23ab29cf2ed78d3be42dc88503b3ee12_1440w.jpg)

然后依次恢复透明度和三维高斯分布（GIF）：

![动图封面](https://pica.zhimg.com/v2-aef0a63984d2a3f021a67ebf9081618c_b.jpg)
...
![](https://pic3.zhimg.com/v2-3763d95c374602b22bb98a691849bb58_1440w.jpg)
...
- **[diff-gaussian-rasterization](https://link.zhihu.com/?target=https%3A//github.com/graphdeco-inria/diff-gaussian-rasterization)** ：由官方提供的使用 Cuda 的 3DGS 光栅化库。
- **[3DGS.cpp](https://link.zhihu.com/?target=https%3A//github.com/shg8/3DGS.cpp)** ：参考 Cuda 版本，通过 Vulkan 计算管线实现的 3DGS 光栅化库。
- 使用图形渲染管线，具体的步骤是：在几何层面将高斯基元视为一个平面四边形，在着色阶段去绘制二维高斯分布。这种做法具有较高的通用性，这是一些开源的参考：
- **[fast-gaussian-rasterization](https://link.zhihu.com/?target=https%3A//github.com/dendenxu/fast-gaussian-rasterization)** ：使用 Python OpenGL借助几何着色器实现的版本。
	- **[splat](https://link.zhihu.com/?target=https%3A//github.com/jakobbbb/splat)** ：使用 C++ OpenGL通过实例化渲染的版本。
	- **[QEngineUtilities](https://link.zhihu.com/?target=https%3A//github.com/Italink/QEngineUtilities/blob/main/Source/Core/Source/Private/Render/Component/QGaussianSplattingPointCloudRenderComponent.cpp)** ：包含使用 C++ QRhi 通过实例化渲染的版本。

以上就是3dgs的全部内容，如果对数学细节感兴趣，这里有一篇很好的资料：

- 高斯分布综合概述： [medium.com/data-science](https://link.zhihu.com/?target=https%3A//medium.com/data-science/a-comprehensive-overview-of-gaussian-splatting-e7d570081362)
...
- [“重建”一座山只需10分钟？神奇的NeRF技术能怎么用！ - 影视飓风](https://link.zhihu.com/?target=https%3A//www.bilibili.com/video/BV1gF4m1u74D)

由于标题存在一定的误导性，导致UP在复现视频中的流程，看到实际效果之后，直呼 NeRF 大有可为 ！被啪啪打脸之后才发现这里面所采用的技术实际上是3DGS，而非NeRF。

鉴于一个潜在的开发需求，驱动着UP对3DGS展开了深入的研究：

- 因为实际进入到游玩的开放场景需要加载大量的资源（数十GB），大多数游戏都会使用一个Launch/Loading关卡，延缓用户操作，提前开始加载。为了让这个过程更加无感，我希望Launch关卡的背景是整个开放场景的动态俯瞰，有点类似于 [这个效果](https://link.zhihu.com/?target=https%3A//www.bilibili.com/video/BV1tfWne4Eia/%3Fshare_source%3Dcopy_web%26vd_source%3Ddfd2f1bf643a3e0722141847666af060%26t%3D587) ，当加载完成时可以从俯瞰视角平滑过渡到玩家视角，因此需要一个全场景的轻量级视觉代理，因为实际场景在引擎中具有庞大的几何数据以及复杂的材质效果，使用传统的网格烘培简化很难完成，而通过一些常用的图像合成手段往往会存在一些瑕疵，因此想到了通过三维重建技术来制作这个视觉代理。

与传统的图形基元相比，3DGS存在很多的缺点：

![](https://pic4.zhimg.com/v2-ca6290e9ae4ca004cb788f4edb0da547_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-c2d2875d3c012c508c8850c8b718983e_1440w.jpg)
...
![](https://picx.zhimg.com/v2-81f08c481881c5282f3f135b8c7e94d1_1440w.jpg)

通常使用 `50%` 的简化幅度能较好的平衡性能与视角质量，但这种策略在一些细节特征复杂，几何形状不规则的模型上，得到的效果并不理想。

例如一个一百万三角形数量的模型，如果我们按照50%的简化效率，通常需要 `14级LOD` ，而这显然会产生大量的数据，且LOD的调度开销会很高，为了限制LOD的数量在 `8` 以内，简化效率必须提升到 `75%` ，就像这样：

![](https://pic3.zhimg.com/v2-d043822792e5d29f6a880def4ef48c2c_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-67cae19b05b5da9f68c13edaa14f7df5_1440w.jpg)

关于这些策略的对比，在之前的植被优化的文章中有简单提及，详见 ：

- [zhuanlan.zhihu.com/p/71](https://zhuanlan.zhihu.com/p/713731229)

这些策略有着不同的优缺点，适用于不同的使用需求，虽然它们确实能大幅度减少性能开销，但通常只适用于单个结构较为“匀称”的模型，对于结构复杂的组合模型，类似于这种，效果就有些差强人意了：

![](https://pic3.zhimg.com/v2-054a6167213e38f880cdc7c36c55a902_1440w.jpg)

动图查看：https://italink.github.io/ModernGraphicsEngineGuide/04-UnrealEngine/Resources/Imposter.gif

而 3DGS 的流程本身就是通过梯度下降去逼近原始图像，虽然在高精度下，GS的数量级很容易超过原始网格的三角形数量，但在低精度时，相比传统的优化手段，越是复杂的模型，3DGS的拟合效果越好：

![](https://picx.zhimg.com/v2-d16e687432981c4a8114cb7e51c3b6e7_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-2f66e1b1eb2dc88eec7b8375d1f06834_1440w.jpg)

![](https://pic2.zhimg.com/v2-d8eff0690e64a4ab7df02fb6535e00ef_1440w.jpg)

动图查看：https://italink.github.io/ModernGraphicsEngineGuide/04-UnrealEngine/Resources/Impos11ter.gif

有了这些理论的支撑，于是UP去做了一些工程化的尝试，虽然过程有点艰难，感兴趣的小伙伴可以下载下来试试看：

- [github.com/Italink/Cosm](https://link.zhihu.com/?target=https%3A//github.com/Italink/Cosmos/releases/tag/v0.0.1)
...
![](https://pic1.zhimg.com/v2-469a3d7b4dbb95805b80c1b229293e28_1440w.jpg)

![](https://pic3.zhimg.com/v2-e414ad1ecabe8092527a1bb96eb6e108_1440w.jpg)
...
- [Github | Gaussian Splatting For Unreal Engine - Italink](https://link.zhihu.com/?target=https%3A//github.com/Italink/GaussianSplattingForUnrealEngine)

> 开发不易，如果这个插件对你有帮助的话，帮忙 [点个Star](https://link.zhihu.com/?target=https%3A//github.com/Italink/GaussianSplattingForUnrealEngine.git) 呀 0.0：

![](https://pic4.zhimg.com/v2-6590f8866cd750782d935ce2f75b9ed1_1440w.jpg)
...
- `GaussianSplattingPointCloud` ：自定义的UE资产，包含3DGS点云的结构。
		...
		- `Spz` ：3D GS 的压缩算法，来源于 [github.com/nianticlabs/](https://link.zhihu.com/?target=https%3A//github.com/nianticlabs/spz)
...
编辑于 2025-05-21 11:24・广东[虚幻引擎](https://www.zhihu.com/topic/19824201)[3DGS](https://www.zhihu.com/topic/30246750)[计算机图形学](https://www.zhihu.com/topic/19613730)[轮廓固定到底是怎么“固定”的？通过注射是怎么达到提升、紧致的效果?把这个基础的原理讲清楚。](https://zhuanlan.zhihu.com/p/7550392148)
