---
title: candycat1992/Unity_Shaders_Book at unity_2017_1
source: https://github.com/candycat1992/Unity_Shaders_Book/tree/unity_2017_1
tags:
  - clippings
  - Unity
  - shader
...
本项目是书籍《Unity Shader入门精要》的配套源代码。最新版本请移步 [本项目的Github页面](https://github.com/candycat1992/Unity_Shaders_Book) 。

## 源码下载

我们推荐您从Github上clone项目源码并及时检查更新。如果下载速度过慢可以移步 [百度网盘地址](https://pan.baidu.com/s/1hr4crsS) 。

纸质版书籍可在以下链接购买： [亚马逊](https://www.amazon.cn/Unity-Shader%E5%85%A5%E9%97%A8%E7%B2%BE%E8%A6%81-%E5%86%AF%E4%B9%90%E4%B9%90/dp/B01G95GMU6/ref=sr_1_1?s=books&ie=UTF8&qid=1464607131&sr=1-1&keywords=unity+shader%E5%85%A5%E9%97%A8%E7%B2%BE%E8%A6%81) 、 [当当](http://product.dangdang.com/23972910.html) 、 [京东](http://item.jd.com/11927199.html)

## 随书彩图

我们提供了包含书中所有插图的彩色版插图集锦： [HTML](http://candycat1992.github.io/unity_shaders_book/unity_shaders_book_images.html) ， [PDF](http://candycat1992.github.io/unity_shaders_book/unity_shaders_book_images.pdf) 。

## 第四章勘误

由于数学章是全书的重要基础，我们决定把第四章公开，来及时让读者获取最新的第四章数学章的内容： [PDF](http://candycat1992.github.io/unity_shaders_book/unity_shaders_book_chapter_4.pdf) 。

**注意** ：我们可能会根据读者勘误随时更新该文档，内容和页号可能会与读者手中的版本不同，实体书中的勘误会在每次重印时进行修正。

## 读者反馈和勘误

尽管我们在本书的编写过程中多次检查内容的正确性，但不可避免书中仍然会出现一些错误，欢迎读者批评指正。任何关于本书内容、源码等方面的问题，欢迎读者反映到本书源码所在的 [Github讨论页](https://github.com/candycat1992/Unity_Shaders_Book/issues) ，也可以发邮件（ [lelefeng1992@gmail.com](mailto:lelefeng1992@gmail.com) ）联系笔者。

关于目前 **已发现的错误** 我们会及时在网上更新： [勘误列表](http://candycat1992.github.io/unity_shaders_book/unity_shaders_book_corrigenda.html) 。

我们也维护了读者反馈的问题列表： [FAQ](http://candycat1992.github.io/unity_shaders_book/unity_shaders_book_faq.html) 。
...
**分支链接** ： [unity\_5\_4](https://github.com/candycat1992/Unity_Shaders_Book/tree/unity_5_4)

Unity 5.4对Shader部分进行了一些比较大的更新，比较明显的变化有：

- 使用了unity\_XXX来代替原有的XXX变换矩阵，例如\_Object2World被替换成了unity\_ObjectToWorld，\_World2Object被替换成了unity\_WorldToObject（均在UnityShaderVariables.cginc文件中被声明），\_LightMatrix0被替换成了unity\_WorldToLight（在AutoLight.cginc文件中被声明）。
- 使用了一些内置函数来代替某些运算，例如mul(UNITY\_MATRIX\_MVP,*)相关计算被替换成了UnityObjectToClipPos(*)。
...
**分支链接** ： [unity\_2017\_1](https://github.com/candycat1992/Unity_Shaders_Book/tree/unity_2017_1)

Unity 2017对Shader部分没有较大更新，我们主要做了以下更改来消除升级造成的Shader Warning信息：

- 使用内置的UnityObjectToViewPos(\*)函数来代替mul(UNITY\_MATRIX\_MV, \*)对顶点进行变换。
...
| 文件夹 | 说明 |
| --- | --- |
| Assets/Scenes | 包含了各章对应的场景，每个章节对应一个子文件夹，例如第七章所有场景所在的子文件夹为Assets/Scenes/Chapter7。每个场景的命名方式为Scene\_章号\_小节号\_次小节号，例如7.2.3节对应的场景名为Scene\_7\_2\_3。如果同一个小节包含了多个场景，那么会使用英文字母作为后缀依次表示，例如7.1.2节包含了两个场景Scene\_7\_1\_2\_a和Scene\_7\_1\_2\_b。 |
